...
def main():
    ...
    parser.add_argument("--reference", nargs='+', type=str, help = "Ensure that given genome is acyclic by deleting all paralogy edges in postprocessing, also do not mask its PAF mappings. If more than one specified, then only first is acyclic, but all will end up as reference-sense in the output vg (if --outVG specified)")
...
def make_align_job(options, toil, config_wrapper=None, chrom_name=None):
    ...
    align_job = Job.wrapJobFn(cactus_align,
                              ...
                              referenceEvents=options.reference,
...
def export_vg(job, hal_id, config_wrapper, doVG, doGFA, referenceEvents, checkpointInfo=None, resource_spec = False,
    ...
    if referenceEvents:
        for referenceEvent in referenceEvents:
